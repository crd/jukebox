...
def build_cardless_speechlet_response(
    output, reprompt_text, should_end_session, speech_type="PlainText"
):
    text_or_ssml = "text"
    if speech_type == "SSML":
        text_or_ssml = "ssml"
    return {
        "outputSpeech": {"type": speech_type, text_or_ssml: output},
        "reprompt": {"outputSpeech": {"type": "PlainText", "text": reprompt_text}},
        "shouldEndSession": should_end_session,
    }


def build_audio_or_video_response(
    title, output, should_end_session, url, token, offsetInMilliseconds=0
):
    if video_or_audio == [True, "video"]:
        return build_video_response(title, output, url)
    else:
        return build_audio_speechlet_response(
            title, output, should_end_session, url, token, offsetInMilliseconds=0
        )


def build_audio_speechlet_response(
    title, output, should_end_session, url, token, offsetInMilliseconds=0
):
    return {
        "outputSpeech": {"type": "PlainText", "text": output},
...
def build_cardless_audio_speechlet_response(
    output, should_end_session, url, token, offsetInMilliseconds=0
):
    return {
        "outputSpeech": {"type": "PlainText", "text": output},
        "directives": [
...
def build_audio_enqueue_response(
    should_end_session, url, previous_token, next_token, playBehavior="ENQUEUE"
):
    to_return = {
        "directives": [
            {
    ...
    if playBehavior == "ENQUEUE":
        to_return["directives"][0]["audioItem"]["stream"][
        ...
        "directives": [
            {
                ...
                "videoItem": {"source": url, "metadata": {"title": title,}},
            }
        ],
    }
...
def lambda_handler(event, context):
    if "expires" in environ and int(datetime.strftime(datetime.now(), "%Y%m%d")) > int(
        environ["expires"]
    ):
...
def create_list(event, list_title, list_items=[]):
    ...
    if not headers:
        return False
    data = {"name": list_title, "state": "active"}
...
def update_list_item(
    event, listId, itemId, itemValue, itemVersion, itemStatus="completed"
):
    headers = get_headers(event)
    if headers:
        data = {"value": itemValue, "status": itemStatus, "version": itemVersion}
        url = (
...
def get_videos_from_url(url):
    t = re.search(r"youtube.com\/watch\?v=.*&list=([^&]+)", url, re.I)
    if t:
    ...
    t = re.search(r"youtube.com\/playlist\?list=([^&]+)", url, re.I)
    if t:
        logger.info("match on t2")
        playlist_id = t.groups()[0]
        videos = get_videos_from_playlist(playlist_id)
        return videos, strings["playlist"], get_title(playlist_id, "playlists")
    t = re.search(r"youtube.com\/watch\?v=([^&]+)", url, re.I)
    if t:
        logger.info("match on t3")
        video_id = t.groups()[0]
        return [video_id], strings["video"], None
    t = re.search(r"youtu.be\/([^&]+)", url, re.I)
    if t:
        logger.info("match on t4")
        video_id = t.groups()[0]
        return [video_id], strings["video"], None
    t = re.search(r"youtube.com\/channel\/([^&]+)", url, re.I)
    if t:
    ...
    t = re.search(r"youtube.com\/user\/([^&]+)", url, re.I)
    if t:
        logger.info("match on t6")
        username = t.groups()[0]
        channel_info = youtube_channel_search(username)
        if (
            "items" in channel_info
            and len(channel_info["items"]) > 0
            and "id" in channel_info["items"][0]
        ):
...
def get_welcome_response(event):
    ...
    list_created = create_list(
        ...
        [
            ...
            "that song I like | https://youtu.be/gJLIiF15wjQ",
    ...
    return build_response(
        build_cardless_speechlet_response(
            speech_output, reprompt_text, should_end_session, "SSML"
        )
    )
...
def illegal_action():
    ...
    return build_response(
        build_short_speechlet_response(speech_output, should_end_session)
    )
...
def video_search(query=None, relatedToVideoId=None, channelId=None):
    try:
        search_response = youtube_search(
            query, "video", 50, relatedToVideoId, channelId
        )
    except Exception:
...
def my_playlists_search(query, sr, do_shuffle="0"):
    ...
    if len(search_response.get("items")) == 0:
        ...
        search_response["items"] = sorted(
            search_response["items"], key=lambda k: k["ratio"], reverse=True
        )
    for playlist in range(sr, len(search_response.get("items"))):
        if "playlistId" in search_response.get("items")[playlist]["id"]:
            playlist_id = search_response.get("items")[playlist]["id"]["playlistId"]
...
def my_latest_video():
    ...
    if channel_id is None:
        return build_response(
            build_short_speechlet_response(strings["nochannelid"], True)
        )
    search_response = youtube_search(
        None, "video", 50, channel_id=channel_id, order="date"
    )
    videos = []
    for search_result in search_response.get("items", []):
        if "videoId" in search_result["id"]:
...
def channel_search(query, sr, do_shuffle="0"):
    ...
    while "nextPageToken" in data and len(videos) < 200:
        next_page_token = data["nextPageToken"]
        search_response = youtube_search(
            query, "video", 50, channel_id=channel_id, pageToken=next_page_token
        )
...
def get_url_and_title(id):
    if "youtube_dl" in environ and (
        environ["youtube_dl"].lower() == "true" or "http" in environ["youtube_dl"]
    ):
...
def get_url_and_title_youtube_dl(id, retry=True):
    if "youtube_dl" in environ and "http" in environ["youtube_dl"]:
        ...
        if (
            "proxy_enabled" in environ
            and "proxy" in environ
            and environ["proxy_enabled"].lower() == "true"
        ):
        ...
        try:
            ...
            if (
                "youtube_dl_error_mirror" in environ
                and "http" in environ["youtube_dl_error_mirror"]
            ):
...
def get_url_and_title_pytube(id, retry=True):
    ...
    if (
        "proxy_enabled" in environ
        and "proxy" in environ
        and environ["proxy_enabled"] == "true"
    ):
...
def get_live_video_url_and_title(id):
    ...
    try:
        ...
        r = requests.get(u)
        a = re.search(r"https:[%\_\-\\\/\.a-z0-9]+m3u8", r.text, re.I)
        url = a.group().replace("\\/", "/")
        logger.info(url)
        t = re.search(r"<title>(.+) - youtube</title>", r.text, re.I)
        if t:
            title = t.groups()[0]
        video_or_audio[1] = "video"
        return url, title
...
def yes_intent(event):
    ...
    if (
        not sessionAttributes
        or "intent" not in sessionAttributes
        or "sr" not in sessionAttributes
    ):
        return build_response(
            build_cardless_speechlet_response(strings["gonewrong"], None, True)
        )
...
def search(event):
    ...
    if (
        intent_name == "ShuffleIntent"
        or intent_name == "ShufflePlaylistIntent"
        or intent_name == "ShuffleChannelIntent"
        or intent_name == "ShuffleMyPlaylistsIntent"
    ):
        playlist["s"] = "1"
    playlist["l"] = "0"
    videos, playlist_channel_video, playlist_title = check_favorite_videos(
        event, query, playlist["s"]
    )
    if videos == []:
        if (
            intent_name == "PlaylistIntent"
            or intent_name == "ShufflePlaylistIntent"
            or intent_name == "NextPlaylistIntent"
        ):
            videos, playlist_title, playlist["sr"], errorMessage = playlist_search(
                query, sr, playlist["s"]
            )
            playlist_channel_video = strings["playlist"]
        elif (
            intent_name == "SearchMyPlaylistsIntent"
            or intent_name == "ShuffleMyPlaylistsIntent"
        ):
            videos, playlist_title, playlist["sr"] = my_playlists_search(
                query, sr, playlist["s"]
            )
            playlist_channel_video = strings["playlist"]
        ...
        if videos is False:
            return build_response(
                build_cardless_speechlet_response(errorMessage, None, True)
            )
    if videos == []:
        return build_response(
            build_cardless_speechlet_response(strings["novideo"], None, True)
        )
    if len(videos) == 1:
        video_or_audio[1] = "video"
    next_url = None
    for i, id in enumerate(videos):
        if (
            playlist_channel_video != strings["video"]
            and (datetime.now() - startTime).total_seconds() > 8
        ):
            return build_response(
                build_cardless_speechlet_response(
                    playlist_channel_video
                    + " "
                    + playlist_title
                    + " "
                    + strings["notworked"],
    ...
    if next_url is False:
        return build_response(
            build_short_speechlet_response(strings["throttled"], True)
        )
    ...
    card_title = "Youtube"
    return build_response(
        build_audio_or_video_response(
            card_title, speech_output, should_end_session, next_url, next_token
        )
    )
...
def stop():
    ...
    return build_response(
        build_stop_speechlet_response(speech_output, should_end_session)
    )
...
def nearly_finished(event):
    ...
    if title is None:
        ...
        if playlist["i"] != "ShuffleMyPlaylists":
            return do_nothing()
        videos, playlist_title, playlist["sr"] = my_playlists_search(
            playlist["query"], int(playlist["sr"]), playlist["s"]
        )
        for i, id in enumerate(videos):
            playlist["v" + str(i)] = id
            if next_url is None:
    ...
    if next_url is False:
        return do_nothing()
    return build_response(
        build_audio_enqueue_response(
            should_end_session, next_url, current_token, next_token
        )
    )
...
def play_more_like_this(event):
    should_end_session = True
    if (
        "AudioPlayer" not in event["context"]
        or "token" not in event["context"]["AudioPlayer"]
    ):
        speech_output = strings["nothingplaying"]
        return build_response(build_short_speechlet_response(speech_output, True))
    current_token = event["context"]["AudioPlayer"]["token"]
    ...
    speech_output = strings["playing"] + " " + title
    return build_response(
        build_cardless_audio_speechlet_response(
            speech_output, should_end_session, next_url, next_token
        )
    )
...
def skip_by(event, direction):
    ...
    if hours == 0 and minutes == 0 and seconds == 0:
        speech_output = strings["sorryskipby"]
        return build_response(build_short_speechlet_response(speech_output, True))
    current_offsetInMilliseconds = event["context"]["AudioPlayer"][
        "offsetInMilliseconds"
    ]
    skip_by_offsetInMilliseconds = direction * (
...
def say_timestamp(event):
    ...
    if "offsetInMilliseconds" in event["context"]["AudioPlayer"]:
        current_offsetInMilliseconds = int(
        ...
        seconds = (
            current_offsetInMilliseconds - hours * 3600000 - minutes * 60000
        ) / 1000
...
def failed(event):
    ...
    return build_response(
        build_audio_enqueue_response(
            should_end_session, next_url, current_token, next_token, playBehavior
        )
    )
...
def skill_expired():
    ...
    return build_response(
        build_cardless_speechlet_response(speech_output, "", True, "SSML")
    )
